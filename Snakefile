...
if not "EXPERIMENTS" in locals():
    EXPERIMENTS = []
    for f in os.listdir('experiments'):
        fn, ext = os.path.splitext(f)
        if ext == '.py' and '_S_' in fn:
...
rule createNetwork:
    ...
    shell:
        'python src/snakemake_network.py {input} {output}'
...
rule simulateNetwork:
    ...
    shell:
        'python src/snakemake_simulation.py {input} {output} 16'
...
rule analyzeNetwork:
    ...
    shell:
        'python src/snakemake_analysis.py {input} {output} {BASE_PATH}'
