...
class SynapseNumbers():
    ...
    def __init__(self, connectivity, NN, con_path, vol_path, FLN, rho_syn,
        ...
        if connectivity == 'HcpDesikanKilliany':
            ...
            # print(os.getcwd())
            # NOS.to_csv("NOS.csv")
            # relSynapses.to_csv("relSynapses.csv")
            # print(f'relSynapses.sum(axis=1) = {relSynapses.sum(axis=1)}')
    ...
    def calcDirectionality(self, SLN_FF, SLN_FB):
        ...
        def tmp(val, SLN_FF, SLN_FB):
            if val < SLN_FB:
                ...
                return 'FF'
        d = self.SLN.apply(lambda col: col.map(lambda x: tmp(x, SLN_FF, SLN_FB)))
